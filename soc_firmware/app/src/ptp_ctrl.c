...
LOG_MODULE_REGISTER(ptp_ctrl, LOG_LEVEL_INF);
 ...
 *
 * Zephyr's public PTP header (zephyr/net/ptp.h) carries no status or
 * configuration API, so this backend reads the stack's internal datasets
 * (subsys/net/lib/ptp is on the include path for this build). Writes go
 * into the live default/port datasets — announce messages and the BTCA
 * consume them on the fly — followed by a state-decision request and an
 * eventfd kick of the PTP thread.
 * ================================================================ */
