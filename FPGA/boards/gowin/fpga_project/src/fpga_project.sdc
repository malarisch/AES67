...
create_clock -name clk_27MHz -period 37 -waveform {0 18} [get_ports {clk_27m}] -add
create_generated_clock -name nco_mclk -source [get_ports {clk_27m}] -master_clock clk_27MHz -divide_by 40 -multiply_by 36 [get_nets {top_inst/aes67_top_inst/wc_mclk}]
create_generated_clock -name tdm_bclk_f_d -source [get_ports {clk_27m}] -master_clock clk_27MHz -divide_by 80 -multiply_by 36 -invert [get_pins {top_inst/aes67_top_inst/audioclocks_inst/bclk_f_r_s0/Q}]
...
set_clock_uncertainty 4 -setup -from [get_clocks {nco_mclk}] -to [get_clocks {clk_27MHz}] 
set_false_path -from [get_clocks {rmii_refclk}] -to [get_clocks {clk_27MHz}] 
set_false_path -from [get_clocks {clk_27MHz}] -to [get_clocks {rmii_refclk}] 
...
set_false_path -from [get_clocks {mac_clk}] -to [get_clocks {clk_27MHz}] 
set_false_path -from [get_clocks {clk_27MHz}] -to [get_clocks {mac_clk}] 
set_clock_groups -asynchronous -group [get_clocks {clk_27MHz nco_mclk tdm_bclk_f_d}] -group [get_clocks {rmii_refclk mac_clk}]
