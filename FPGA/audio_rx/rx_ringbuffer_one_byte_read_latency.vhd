...
architecture Behavioral of rx_ringbuffer is
    ...
    type t_s_parsePacket is (s_Idle, s_readHeader_wait, s_readHeader, s_prepare, s_readSampleData_wait, s_readSampleData, s_End);
            ...
            case packetParserState is
                when s_Idle =>
                    if packet_ready_i_sync2 /= packet_ready_i_sync3 then
                        ...
                        packetParserState <= s_readHeader_wait;
                        packet_read_index <= 16;
                        eth_read_addr_o <= to_unsigned(16, 11);
                    end if;

                when s_readHeader_wait =>
                    -- eth_ram has 2-cycle read latency (addr registered,
                    -- then data registered on sys_clk).  Pre-request next
                    -- address so pipeline stays filled when we enter
                    -- s_readHeader.
                    eth_read_addr_o <= to_unsigned(17, 11);
                    packetParserState <= s_readHeader;

                when s_readHeader =>
                    -- eth_read_data_i now holds data for the address
                    -- requested 2 cycles ago.  Each step pre-requests
                    -- the address needed 2 steps later.
                ...
                when s_readSampleData =>
                    if packet_read_index >= to_integer(unsigned(current_packet_length)) + 14 then
                        ...
                        -- Write current byte to RAM (eth_read_data_i is now valid
                        -- for the address presented in the previous cycle)
                        sample_wr_addr <= wr_addr_current;
                        sample_wr_data <= eth_read_data_i;
                        sample_wr_en <= '1';

                        -- Advance ETH read address (2-cycle latency: request 2 ahead)
                        eth_read_addr_o <= to_unsigned(packet_read_index + 2, 11);
                        packet_read_index <= packet_read_index + 1;

                        -- Compute next write address incrementally
                        ...
                        -- Stay in readSampleData — the address we just set will
                        -- be valid next cycle, so we can read+write continuously.
                        -- (no extra wait needed for consecutive reads)
                    end if;
