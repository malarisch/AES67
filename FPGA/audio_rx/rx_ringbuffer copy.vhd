...
entity rx_ringbuffer is
    ...
    (
        ...
        TDM_OUTPUTS:  integer := 4; -- amount of output pins
...
architecture Behavioral of rx_ringbuffer is
    ...
    signal tdm_byte_latch : t_tdm_latch  := (others => (others => '0'));
    signal tdm_channel_bit_counter : unsigned(4 downto 0);
    signal tdm_channel_counter : unsigned(clog2(TDM_CHANNELS)-1 downto 0);
...
tdm_out_parallel_proc_gen: if (PARALLEL_OUT = false) generate
    -- generate the base read pointer
    sample_rd_ptr <= unsigned(media_clock_i(SAMPLE_IDX_BITS - 1 downto 0)) & to_unsigned(0, SAMPLE_SHIFT);
    tdm_ram_read_proc: process(sys_clk, reset_n)
        variable start_latching : boolean := false;
        variable current_output_index : integer range 0 to TDM_OUTPUTS - 1 := 0;
    begin
        if (reset_n = '0') then

        elsif (rising_edge(sys_clk)) then
            if (bclk_sync_i = '1' and zbclk = '0') then
                if (tdm_channel_bit_counter = d"31") then
                    -- start latching out the next bytes for the multiplexers
                    start_latching := true;
                end if;
                if start_latching then
                    if (current_output_index <= TDM_OUTPUTS) then
                        sample_rd_addr <= sample_rd_ptr
                            + to_unsigned(tdm_channel_counter * CHANNEL_STRIDE + (bytes_per_sample - 1 - (playout_byte + 1)), ADDR_BITS);
                    end if;
                end if;
            end if;
        end if;
    end process;
    -- controll all counters
    tdm_out_ctrl_proc: process (sys_clk) begin
        if (rising_edge(sys_clk)) then
            if (bclk_sync_i = '1' and zbclk = '0') then
            ...
            if (fs_clk_sync_i = '1' and zaudio_sync = '0') then
                tdm_channel_bit_counter <= "0";
                tdm_channel_counter <= "0";
            end if;
        end if;
        end if;
    end process;
    tdmoutgen : for i in 0 to TDM_OUTPUTS - 1 generate
        ...
        begin
            if (rising_edge(sys_clk)) then
                -- use the lower index of the bit counter to get the current bit from the latched bytes
                tdm_out(i) <= tdm_byte_latch(i)(to_integer(tdm_channel_bit_counter(2 downto 0)));
            end if;
        end process;
    end generate;
end generate;
