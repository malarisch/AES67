...
# HyperRAM (driven by LiteX HyperRAM PHY in litex_sys_clk domain, internal timing)
set_false_path -from * -to [get_ports {hbus_clk0_p hbus_clk0_n hbus_rstn hbus_cs2n}]
set_false_path -from * -to [get_ports {hbus_dq[*]}]
set_false_path -from [get_ports {hbus_dq[*]}] -to *
set_false_path -from * -to [get_ports {hbus_rwds}]
set_false_path -from [get_ports {hbus_rwds}] -to *
